...
eess-ts scans every `JsxElement` and `JsxSelfClosingElement` in `.tsx`/`.jsx` files and wraps them in an `ArchJsxElement` model.
...
```typescript
import { project, functions, modules, jsxElement } from '@nielspeter/eess-ts'
...
```typescript
import { STANDARD_HTML_TAGS } from '@nielspeter/eess-ts'
...
eess-ts warns when exclusions become stale (the violation was fixed but the exclusion remains).
