...
| `check:crossval`   | `eess-crossvalidate` | That diagram and the kernel's code stay in agreement (both directions), every test an ADR cites resolves in the real AST, every markdown citation of a `.feature` scenario resolves, and the kernel diagram [embedded in the guide](/architecture) stays in agreement with the code too |
