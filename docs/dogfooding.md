...
| `check:crossval`   | `eess-crossvalidate` | Five bindings hold code and the repo's own docs in agreement: the kernel diagram (both directions, standalone and [embedded in the guide](/architecture)), every ADR-cited test, and every markdown citation of a `.feature` scenario |
| `check:corpus`     | `eess-md`            | 101 markdown docs (plans, ADRs, bugs, guide): cross-links resolve, `path:line` pointers are live, and every ADR's `## Enforcement` table is well-formed                                                                               |
