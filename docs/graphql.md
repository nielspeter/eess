...
```typescript
import { schema } from '@nielspeter/eess-ts/graphql'
...
```typescript
import { schemaFromSDL } from '@nielspeter/eess-ts/graphql'
...
The `graphql` package is optional — eess-ts core works without it. Only install it if you use `eess-ts/graphql`.
