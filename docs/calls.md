...
eess-ts scans every `CallExpression` in the project and wraps it in an `ArchCall` model with precomputed fields. This model gives you uniform access to the call's target object, method name, and arguments regardless of whether the call is a method invocation like `app.get(...)` or a bare function call like `handleError(...)`.
...
**Four shapes it does not read**, every one a silent pass (bug 0331): a callback a NAME refers to (`use(handler)`), which needs the type checker; one in an array or other collection, including the `app.get('/x', [authenticate, handler])` idiom; one held by a getter (`{ get handler() { … } }`); and one nested deeper than three object literals. `within()` reads the same nothing. Use `haveArgumentContaining` for those — it searches an argument whole.
...
No callback argument may contain the matched expression. It reads the same callbacks as `haveCallbackContaining`, so `app.get('/', () => db.query(sql))` is reported under the rule below.
...
```typescript
import { calls, property } from '@nielspeter/eess-ts'
