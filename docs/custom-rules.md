...
eess-ts provides `definePredicate()` and `defineCondition()` for encoding team-specific conventions that aren't covered by built-in rules.
...
```typescript
import type { ArchFunction } from '@nielspeter/eess-ts'
...
```typescript
import { defineCondition, createViolation, classes } from '@nielspeter/eess-ts'
import type { ClassDeclaration } from 'ts-morph'
import type { ArchViolation, ConditionContext } from '@nielspeter/eess-ts'
