...
>
> - [`eess-ts`](/getting-started) — TypeScript source (this site's focus)
> - [`eess-mermaid`](/mermaid) — Mermaid class diagrams
> - [`eess-md`](/markdown) — a Markdown corpus (docs, ADRs, plans)
> - [`eess-crossvalidate`](/crossvalidate) — bind two dialects so drift in either fails the build
> - [eess validates eess](/dogfooding) — how this project uses every dialect on itself, in CI

---

## What eess-ts Can Enforce
...
```typescript
// eess-ts catches this on the PR that introduces it:
modules(p)
  .that()
...
**eess-ts turns these rules into tests.** They run in CI. Violations are caught on the PR that introduces them — not 18 months later during a manual audit.
...
| Capability                                         | eess-ts | dependency-cruiser | eslint-plugin-boundaries |
| -------------------------------------------------- | ------- | ------------------ | ------------------------ |
| Import path rules                                  | ✅      | ✅                 | ✅                       |
...
eess-ts is the guardrail. Rules run in CI. Violations show up inline on the PR with clear messages explaining **what's wrong, why it matters, and how to fix it** — exactly the context an agent needs to self-correct.
