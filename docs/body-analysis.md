...
Body analysis is the signature feature of eess-ts. While other tools check import paths, eess-ts inspects what happens _inside_ function and method bodies -- the actual AST of call expressions, constructor invocations, property access, and arbitrary expressions.
...
```typescript
import { newExpr } from '@nielspeter/eess-ts'
...
```typescript
import { property } from '@nielspeter/eess-ts'
...
```typescript
import { expression } from '@nielspeter/eess-ts'
...
```typescript
import { jsxElement } from '@nielspeter/eess-ts'
...
```typescript
import { jsxText } from '@nielspeter/eess-ts'
...
```typescript
import { typeAssertion } from '@nielspeter/eess-ts'
...
```typescript
import { nonNullAssertion } from '@nielspeter/eess-ts'
...
```typescript
import { calls, within, typeAssertion, nonNullAssertion } from '@nielspeter/eess-ts'
...
```typescript
import { comment, STUB_PATTERNS } from '@nielspeter/eess-ts'
...
- **`functions(p)`** -- checks the code each matched function/arrow/method runs, individually: its body, and what its parameters run — a default, and a destructured or rest parameter's defaults and computed keys. The subjects are a file's functions, the functions inside its namespaces, a class's function members — a class expression's included — and, with `includeObjectLiteralFunctions`, an object literal's function values and accessors (bug 0321). A static block, a class field that holds no function, and a callback passed inline to a call are **not** function subjects: the class and module rules read those
...
```typescript
import { noConsoleLog } from '@nielspeter/eess-ts/rules/security'
...
```typescript
import { noEval } from '@nielspeter/eess-ts/rules/security'
...
```typescript
import { noProcessEnv } from '@nielspeter/eess-ts/rules/security'
...
```typescript
import { noFunctionConstructor } from '@nielspeter/eess-ts/rules/security'
...
- **`expression()` deduplicates ancestor matches.** Since `expression()` walks all AST descendants, parent nodes whose `getText()` contains the same pattern are automatically filtered out — only the deepest matching node is reported. This prevents inflated violation counts (e.g., one `reply.code(400)` producing 10+ violations from ancestor nodes). When several nodes cover exactly the same text — a statement without a semicolon and its expression, a shorthand property and its name, a call's only argument — one of them is reported, once.
