...
## TypeScript (`eess-ts/rules/typescript`)

TypeScript's type system only helps if you actually use it. These rules catch the three most common ways teams silently opt out of type safety: `any` types on class properties (defeats the entire type system for that value), `as` type assertions in method bodies (tells the compiler "trust me" instead of proving correctness), and `!` non-null assertions (ignoring possible null/undefined). ESLint can catch some of these, but eess-ts checks them architecturally — you can scope enforcement to specific layers or class hierarchies.
...
**Tip:** the underlying `typeAssertion()` and `nonNullAssertion()` matchers (from `@nielspeter/eess-ts`) are directly composable with `notContain()` on any entry point, including `within()` callbacks and future entry points. The rule functions above are thin wrappers for the common cases.

## Security (`eess-ts/rules/security`)
...
site of use, and no others: through one of `globalThis`, `window`, `self` or `global`, through a
...
is outside `noProcessEnv`. Nor do the rules read through a type assertion or a non-null assertion —
`(globalThis as any).process.env`, `process!.env` — or a second global object (bug 0308). And a
...
## Error Handling (`eess-ts/rules/errors`)
...
import {
  ...
  moduleNoSilentCatch,
} from '@nielspeter/eess-ts/rules/errors'
```
...
## Architecture (`eess-ts/rules/architecture`)
...
```typescript
import { mustMatchName, mustNotEndWith } from '@nielspeter/eess-ts/rules/naming'
```
...
## Dependencies (`eess-ts/rules/dependencies`)
...
```typescript
import { onlyDependOn, mustNotDependOn, typeOnlyFrom } from '@nielspeter/eess-ts/rules/dependencies'
```
