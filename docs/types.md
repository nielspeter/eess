...
```typescript
import { project, types, not, isString } from '@nielspeter/eess-ts'
...
eess-ts uses the TypeScript type checker to resolve types. This means:
...
```typescript
import { project, types } from '@nielspeter/eess-ts'
