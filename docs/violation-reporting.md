...
When an architecture rule fails, eess-ts produces rich violation messages with code frames, file paths, line numbers, and optional context about why the rule exists and how to fix it.
...
```typescript
// eess-exclude sdk/no-manual-urlsearchparams: builds image transform URL, not list pagination
async getImageUrl() {
...
```typescript
// eess-exclude rule-a, rule-b: shared reason for both rules
doSomething()
```
...
```typescript
import { detectFormat } from '@nielspeter/eess-ts'
...
```typescript
import { detectFormat, isCI } from '@nielspeter/eess-ts'
...
```typescript
import { ArchRuleError } from '@nielspeter/eess-ts'
...
```typescript
import { generateCodeFrame } from '@nielspeter/eess-ts'
