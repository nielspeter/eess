...
import { layeredArchitecture } from '@nielspeter/eess-ts/presets'
...
```typescript
import { describe, it } from 'vitest'
import { workspace, modules, noUnusedExports } from '@nielspeter/eess-ts'
...
eess-ts also runs standalone without a test runner — for pre-commit hooks, CI pipelines, or one-off audits. See the [CLI documentation](/cli) for all commands, options, watch mode, and config file setup.
...
- [What to Check](/what-to-check) — scan the recipe gallery to see what eess-ts can enforce
