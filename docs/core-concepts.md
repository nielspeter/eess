...
This comparison shows what eess-ts replaces. Manual AST traversal is verbose, error-prone, and produces poor error messages. The fluent DSL compresses the same logic into a single readable chain while adding code frames, violation context, and CI-friendly output for free.

Without eess-ts, enforcing architecture means manual AST traversal with ts-morph:

```typescript
// WITHOUT eess-ts: 12 lines of manual AST traversal
...
With eess-ts, the same rule is one fluent chain:

```typescript
// WITH eess-ts: 1 chain
...
```typescript
import { and, or, not, extend, implement, haveDecorator } from '@nielspeter/eess-ts'
...
```typescript
import { withBaseline } from '@nielspeter/eess-ts'
...
```typescript
import { diffAware } from '@nielspeter/eess-ts'
