 ...
 *
 * The one place in the kernel that reaches for `Object.create`/
 ...
 * makes `next._x = ...` safe; a field holding a mutable container still
 * needs the owning class to replace it, which is what each builder's own
 * `copy()` override does on top of this. A deep clone would instead copy a
 * dialect's underlying project graph — the opposite of cheap, and it would
 * break identity comparisons on the elements a dialect selects.
 *
 * Class fields declared with a constructor parameter property (e.g.
 * `private readonly project`) are ordinary instance properties at runtime,
 * so they come across too: a clone is built against the same project,
