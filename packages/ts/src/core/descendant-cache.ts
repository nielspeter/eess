 ...
 * rule per banned API, each carrying `notContain(call(api))`.
 *
 ...
 * `comment()`, which take the broad path.
 *
 * Caching `getDescendants()` would retain every wrapper in every body rather
 * than one kind's worth, which is a materially different memory profile than
 * `descendantsOfKind`'s per-kind slots — so it is its own cache below
 * ({@link allDescendants}), not a side effect of this one.
 *
 * ## Invalidation, which is the whole difficulty
 *
 * A node key is **not** safe on its own: a function node survives an edit to
 * its own body and is not forgotten, so a `WeakMap<Node, …>` would serve the
 * pre-edit list — the same trap `SourceFile` has, except `Node` has no
 * `onModified` of its own to escape it.
 *
 * So entries are grouped **by source file** and dropped wholesale when that
 * file is modified. One listener per file, guarded by a `WeakSet`.
 *
 * ## Forgotten descendants, and the limit that remains
 *
 * `node.forget()` and `project.forgetNodesCreatedInBlock()` forget nodes
 * without modifying the file, so `onModified` never fires. The **key** node
 * stays live and unforgotten while the walked **descendants** are gone — so
 * an early `node.wasForgotten()` check, which is what this module shipped
 * first, guards the case that was already harmless and misses the one that
 * breaks.
 *
 * {@link usable} therefore validates the cached list's endpoints on every hit
 * and re-walks when they are gone. What remains uncovered is interleaved
 * forget points leaving a partially-forgotten list; ts-morph forgets a
 * created-in-block set atomically, so that is not a shape its own mechanism
 * produces. `resetProjectCache()` clears the cache outright and is the escape
 * hatch for a consumer doing explicit node memory management.
 */
 ...
 * `registerCacheReset` replaces the maps.
 */
...
export function descendantsOfKind(node: Node, kind: SyntaxKind): readonly Node[] {
  ...
  // Re-walking is the correct answer — the nodes are gone, so the list is
  // simply wrong.
 ...
 * `ForgetfulNodeCache` already holds every wrapper it creates for the lifetime
 * of the `SourceFile`, so the array adds N references, not N objects.
 */
