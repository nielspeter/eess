...
import { isProjectRelative } from '@nielspeter/eess'
 ...
 * before them is located in the path literally — which is what makes those
 * routes resolve correctly. Cutting at `*`/`?` matters for partial wildcards
 ...
 *
 * Deriving them from differently-normalized inputs is a real, live bug (found
 * live in eess during plan 0147's reconciliation, matching ts-archunit's own
 * bug 0009): the old code appended a trailing wildcard segment to the
 * AUTHORED glob and derived `baseDir` from the authored glob's own last `/`,
 * so a glob whose final
 * segment already carried a wildcard AND a trailing slash —
 * `'src/features/*\/'` — computed `baseDir = 'src/features/*\/'`, a string
 * containing a literal `*` that can never appear in a real file path.
 * `filePath.indexOf(baseDir)` was then always `-1`, so `resolveByMatching`
 * silently returned **zero slices** for that spelling, always — not an
 * edge case, the exact form the docstring's own trailing-slash example used.
 * Both derivations now come from the same `normalized` string, which removes
 ...
 *
 * Redundant and therefore stripped: a leading `**\/` (patterns are matched
 * against absolute paths anyway), a leading `./` (never present in an
 * absolute path), and a trailing `/` once the glob already contains a
 * wildcard segment.
 *
 * A trailing `/` on a wildcard-free glob becomes an explicit `*`, so
 * `'src/features/'` ("the things inside features") is exactly
 * `'src/features/*'`. These are all equivalent — `'src/features/*'`,
 * `'src/features/*\/'`, `'src/features/'`, `'**\/src/features/*'`,
 * `'./src/features/*'` — while `'src/features'` (no slash) is deliberately
 * different: its final segment is a name filter, yielding one slice.
 */
function parseMatchingGlob(glob: string): MatchingGlob {
  let normalized = glob
  // Loop over BOTH redundant prefixes together: './' and '**/' can appear in
  // either order ('**/./src/*'), and stripping one kind to exhaustion before
  // the other left a residual that made the whole glob unmatchable.
  while (normalized.startsWith('./') || normalized.startsWith('**/')) {
    normalized = normalized.startsWith('./') ? normalized.slice(2) : normalized.slice(3)
  }

  // A trailing '/' is redundant once the glob already contains a wildcard
  // segment: that wildcard is the slice level, and the pattern appends its
  // own '*/**'. Left in, it demanded one directory MORE than baseDir
  // implies, so every file sitting directly in the matched directory was
  // silently dropped ('src/feature-*/' lost each feature's index.ts;
  // 'packages/*/src/' lost everything directly in src).
  //
  // With no wildcard anywhere the slash is meaningful and stays:
  // 'src/features/' means "the directories inside features". The test is
  // about the glob as a whole, not the final segment's shape — `[slug]` and
  // `(marketing)` are real directory names, and testing them for
  // metacharacters is what broke them before.
  if (normalized.endsWith('/')) {
    const withoutSlash = normalized.slice(0, -1)
    const hasWildcardSegment = withoutSlash.split('/').some((seg) => WILDCARD_CHARS.test(seg))
    // With a wildcard already present that wildcard IS the slice level, so
    // the slash is noise. Otherwise 'dir/' means "the things inside dir",
    // which is exactly 'dir/*' — make it explicit so both spellings take the
    // same path and a flat directory (files, no subdirectories) resolves for
    // both.
  ...
  // The slice level is the first wildcard segment; everything literal before
  // it is the directory prefix we locate in the path. A trailing '/' has
  // already been turned into an explicit '*', so the remaining no-wildcard
  // case is a bare 'src/feature-', where the final segment is a NAME FILTER
  // rather than a directory — which is what distinguishes it from
  // 'src/features/'.
  ...
  // The pattern keeps the glob's own shape (interior segments included) so
  // that `packages/*/src/*` still requires the `/src/` level, and `dir/*`
  // still matches files sitting directly in `dir` as well as subdirectories.
  // Only `baseDir` is segment-derived — it is the part that has to be
  // findable in a real path.
 ...
 *
 * The segment following the glob's literal prefix names each slice. That
 * segment is a **directory** when files are nested under it, and otherwise
 * each matching **file** name — `matching('src/features/*')` over
 * `features/billing/order.ts` yields a `billing` slice, while over a flat
 * `services/order.service.ts` it yields an `order.service.ts` slice.
 *
 * Every spelling of the same intent agrees: `'src/features/*'`,
 * `'src/features/*\/'`, `'src/features/'` and `'**\/src/features/*'` are
 * equivalent.
 *
...
export function resolveByMatching(project: ArchProject, glob: string): Slice[] {
  ...
  // No literal directory prefix (e.g. '*', '**', 'src' with no '/'). Bail
  // rather than search for '' in the path: `indexOf('')` is 0, so the "slice
  // name" would be the path's first segment — empty on POSIX (harmless by
  // accident) but the drive letter on Windows, which would mint ONE slice
  // holding every file and silently pass every inter-slice condition.
  // Returning no slices makes the discovery guard fire instead (ADR-008).
  if (baseDir === '') return []
...
export function resolveByDefinition(project: ArchProject, definition: SliceDefinition): Slice[] {
  ...
  const matchers = entries.map(
    ([name, glob]): {
      ...
      // 'src/api/**' }` failed beside a `shared: ['src/shared/**']` that
      // worked, in the same preset call. Same rule as the predicates (plan
      // 0067 C): relative means from the project root.
