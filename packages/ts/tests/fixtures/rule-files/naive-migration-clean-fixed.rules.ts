// The REMEDY applied — the control for `naive-migration-clean.rules.ts`. Same
// project, same preset, plus `report: 'builders'`. Must load real rules and stay
// green, so a fix that simply failed every zero-violation run cannot pass.
//
// Originally: the naive migration from `@nielspeter/ts-archunit`, over a codebase with NO
