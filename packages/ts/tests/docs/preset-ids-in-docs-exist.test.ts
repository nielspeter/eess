...
function idsMentionedIn(base: string, file: string, unreleasedOnly = false): Map<string, number> {
  ...
  // **The bracket is OPTIONAL, and that is the eess fix.** These patterns arrived
  // from `ts-archunit`, which wrote Keep-a-Changelog headings (`## [0.3.0]`).
  // eess's changelogs are generated by changesets, which writes `## 0.3.0` — so
  // `start` was always -1 here, `endOffset` was -1 too, and the window silently
  // became the ENTIRE FILE: every released section in scope, which the paragraph
  // above says it must not be, because released entries deliberately quote
  // misspelled ids.
  //
  // That paragraph predicted this exact failure ("the guard went vacuous the moment
  // the release commit renamed the heading") and then shipped a pattern that could
  // not match this repo's headings at all.
  const HEADING = /^## \[?(Unreleased|\d)/
  const start = unreleasedOnly ? all.findIndex((l) => HEADING.test(l)) : 0
  const endOffset = unreleasedOnly
    ? all.slice(start + 1).findIndex((l) => /^## \[?\d/.test(l))
    : all.length
