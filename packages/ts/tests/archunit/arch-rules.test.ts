  ...
  it('NO source file may use a type assertion, whatever shape it is written in', () => {
    ...
    gate(
      modules(p).that().satisfy(inProjectSrc()).should().satisfy(moduleNoTypeAssertions()).rule({
        ...
        suggestion:
          'Use a type guard (`value is T`), or narrow with `in`/`typeof` — which is usually already there, since 17 of the 22 casts this rule first found sat directly after the check that made them unnecessary. At a genuine JS-interop boundary, waive it with a `// eess-exclude adr005/no-as-cast-module: <reason>` comment naming the boundary.',
        docs: 'https://github.com/NielsPeter/ts-archunit/blob/main/adr/005-no-any-no-type-assertions.md',
      }),
    ).check()
  })
