/**
 * A `workspace()` judges each package by **its own** tsconfig — plan 0148.
 *
 * `workspace()` builds ONE ts-morph `Project` from the alphabetically-first
 * tsconfig and then only *adds files* from the rest. So before this fix,
 * `sourceFile.getProject().getCompilerOptions()` answered for the tie-break
 * winner whatever package a file was actually in, and `beFreeOfCycles` was
 * wrong in **both** directions depending on a path sort:
 *
 * - primary flag OFF → the flag-`true` package's real cycle **vanished**;
 * - primary flag ON  → the flag-`false` package got a **phantom** cycle,
 *   which reds CI with a remedy ("extract shared code to a lower-level
 *   module") that cannot remediate it, because there is nothing to extract.
 *   An agent handed that restructures working code.
 *
 * Two fixture pairs, because **one pair cannot test this**:
 * `verbatim-module-syntax` and `verbatim-module-syntax-off` always sort with
 * `-off` first (`'-' < '/'`), so that pair only ever exercises one primary.
 * `zz-aa-verbatim-on` / `zz-bb-verbatim-off` force the other. Both directions
 * or the fix passes half the time — a fix that reads the *last* config
 * instead of the first would satisfy a single-order test.
 *
...
describe('workspace() resolves verbatimModuleSyntax per package (plan 0148)', () => {
  it('VACUITY: the four fixtures share source and differ only in the flag', () => {
    // Without this the rows below could pass because two fixtures happen to
    // disagree for an unrelated reason — the whole claim is "the tsconfig is
    // the only difference".
  ...
  it('primary OFF: the flag-true package still reports its cycle', () => {
    // Was a FALSE NEGATIVE — the real cycle vanished, because the primary's
    // `false` was applied to a package whose own tsconfig says `true`.
  ...
  it('primary ON: the flag-false package still reports nothing', () => {
    // Was a FALSE POSITIVE, and the worse half: a phantom cycle that cannot
    // exist at runtime, whose remedy cannot be applied.
  ...
  it('the edge classification itself is per package, not just the cycle verdict', () => {
    // One level below the condition, so a future caller of `edgesOf`
    // inherits the fix rather than re-deriving it. `erasesModuleRequest` is
    // the field this bug corrupted.
    const ws = workspace([cfg(ON), cfg(OFF)])
    ...
    // The same statement, in byte-identical files: erased in the package
    // whose tsconfig says so, and a live module request in the one whose
    // tsconfig says so.
  ...
  it('a UNIFORM workspace is unaffected', () => {
    // Per-file resolution must not break the common case, where every
    // package agrees.
