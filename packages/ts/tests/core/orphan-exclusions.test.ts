...
beforeAll(() => {
  ...
  fs.writeFileSync(
    ...
    '// eess-exclude arch/renamed-away: was valid before the rename\nexport const a = 1\n',
  )
  fs.writeFileSync(
    path.join(dir, 'src/live.ts'),
    '// eess-exclude arch/live: deliberate\nexport const b = 1\n',
  )
  ...
  it('reports each distinct orphan, and does not collapse them', () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        '// eess-exclude arch/gone: a\nexport const a = 1\n' +
          '// eess-exclude arch/gone: b\nexport const b = 2\n',
      )
      fs.writeFileSync(
        path.join(many, 'src/two.ts'),
        '// eess-exclude arch/gone: c\nexport const c = 3\n',
      )
