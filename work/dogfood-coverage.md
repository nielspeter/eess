...
| `typescript/no-type-assertions`       | 39  | **ADOPT + FIX ALL** — first mechanical enforcement of ADR-005 (eslint has no as-ban configured; bare casts exist, e.g. in the kernel's `baseline.ts`). Each site: remove the cast, or sanction as a documented boundary via `eess-exclude` with reason. Zero bare sites at exit. |
...
| `eess-ts`            | `reportViolations`, `dispatchRule`, `validateOverrides`, `throwIfViolations` (removed from every barrel by [plan 0263](./plans/completed/0263-adr-014s-residual-enforcement-rows.md) Phase 5 — this row records what 0089 added, not what ships today), `finishPreset`, `presetConstructsNothingViolation`, `RuleSeverity`, `PresetBaseOptions`, `PresetReportOptions`, `ReportMode`, `ReportOptions` — the whole preset-authoring toolkit `presets/shared.ts` forwards from the kernel, reachable before this pass only via the `/presets` subpath (0088's own root-OR-presets precedent), not the root | added to `packages/ts/src/index.ts` (root). **The `/presets` note was wrong and this row is where a later error came from:** that subpath carried only `dispatchRule` and `validateOverrides`, not the rest. Plan 0263 Phase 5 read this line, believed it, and printed a migration instruction that did not compile — an adopter review caught it, and `finishPreset` was added to the subpath in the same PR. The other four listed here are still root-only |
