...
| [0106](./fixed/0106-no-gate-requires-a-changeset.md)                    | nothing requires a changeset, so a feature merges green and unreleasable — `gherkin-ts` is on `main` and in no published version         | High     | ✅ Fixed → `fixed/` | inbound · adopter integration, verified 2026-08-12                           | #46       |
