...
| [0122](./0122-violations-path-drops-because.md)                         | the `.violations()` path drops `.because`, so every caller-owns-reporting gate ships violations with no rationale                                          | Medium   | 🔴 Draft            | self-found · architect + testing reviews of 0106's fix                       | —         |
