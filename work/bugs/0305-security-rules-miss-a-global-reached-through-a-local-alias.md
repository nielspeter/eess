...
- **State:** Draft — reproduced in both directions, and pinned by KNOWN-GAP tests.
...
- **Origin:** self-found · split from
  ...
  were split from [0297](./fixed/0297-no-process-env-reads-one-spelling-of-an-environment-read.md)
...
An import is a binding too: `import { env } from 'node:process'` reaches the environment under a
name that was never `process`. One answer covers all four rules.
