...
function-valued properties and method shorthands of an object-literal argument; the enforcement
review of 0323 measured `within()` finding the object-literal and parenthesized shapes above. The
callback conditions use `getFunctionBody`, which reads the argument alone.
...
collected every function in an argument, nested ones included, which is not the fix. Two questions
...
yet, and points to `haveArgumentContaining` and `within()`.
...
- [x] each pin goes red when callbacks are collected inside object literals and parentheses (the
      sabotage run of 0323's PR, row R11).
- [ ] a ruling on where a callback is looked for
