...
- **Severity:** **High**. It is a **fail-open on the waiver path**: a teaching example of an exclusion comment, written inside code, silently waives a real finding on the next line.
  ...
  - **No live instance here.** In this repository, the 4 directive lines inside code are all masked.
- **Origin:** self-found · enforcement and testing reviews of a fence-handling fix
- **Reported:** 2026-09-13

## Symptom

The kernel reads exclusion comments in a markdown file through `maskMarkdownCodeSpans` (`packages/core/src/mask-non-comment.ts:171`). The routing is at `packages/core/src/exclusion-comments.ts:391`.
...
- **Closers.** It opens a fence at any indentation (`packages/core/src/mask-non-comment.ts:178`). It closes on any run of the same character and length, even one followed by an info string (`packages/core/src/mask-non-comment.ts:184`). CommonMark does not treat such a line as a closer.
...
2. **No dialect can supply accuracy.** The kernel's rule executor calls `parseExclusionComments(sourceText, filePath)` with no masker (`packages/core/src/execute-rule.ts:151`), although the parser accepts one and runs it first (`packages/core/src/exclusion-comments.ts:399`). A masker also receives only text and a path (`packages/core/src/exclusion-comments.ts:355`), not a document.
...
A real directive in prose must keep waiving.
...
- **A conservative default costs loudly, and sometimes silently.** A kernel spike that never closes a fence once one is seen honoured 0 in-code directives. It had three costs:
...
- [ ] A non-vacuity row: a directive inside a nested fence must not waive.
