...
- **Severity:** Low — every `check:corpus` finding placed on a proposal's `**Ruling:**` line reports a
  line above it. The finding still fires; the reader is sent to a blank line and has to look down for
  the ruling.
...
reports its own line, and so do a bulleted, a quoted and an indented ruling; the ruling and
`**Implements:**` values read are unchanged on all seven shapes. Both sides share the prefix, so both
