...
search takes the broad path (`packages/ts/src/helpers/body-traversal.ts:151`). That path tests every
...
remove the other — measured at zero findings"_ (`packages/ts/src/helpers/body-traversal.ts:160`).
...
(`packages/ts/src/helpers/body-traversal.ts:513`), and each of those is one finding. A by-kind matcher
...
(`packages/ts/src/helpers/body-traversal.ts:391`) counts an initializer's root only when nothing inside
