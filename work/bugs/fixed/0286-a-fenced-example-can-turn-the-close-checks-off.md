...
reads such a fence as raw HTML, so the parser has no code node for it, while 0.6.0's regex — which was
textual — set aside a simple one there (mis-reading a longer one, as it did everywhere else).
