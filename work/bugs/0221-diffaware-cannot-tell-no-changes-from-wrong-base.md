...
- **Origin:** self-found — architect review of [plan 0218](../plans/completed/0218-gate-proposal-acceptance-criteria.md),
  which built `scripts/lib/base-ref.mjs` and noticed the kernel already had base-ref
  awareness with a different disposition.
...
`scripts/lib/base-ref.mjs`, written for plan 0218, takes the opposite position for gates:
`EESS_RELEASE_BASE` → the PR's target → `origin/main` → `main`, and an unresolved base is a
**finding**, never an empty diff. Two base-ref policies now coexist in one repo. That is
defensible — a filter and a gate want different things — but it is undocumented, and the one
that ships to consumers is the one that cannot say which zero it produced.
