...
**Read of the board (2026-08-14).** 0096 shipped (below). Of the remaining six
`High` rows, two can be started today: **0088** and **0089**, neither of which
waits on anything. 0101 needs 0088's folded kernel seam, 0100 needs the three
code plans merged, 0075 waits on adopter feedback, 0079 on a mechanism nobody
has, and every `Low` row on a signal that does not exist yet. The one `Ready`
row, 0091, is also buildable now — so there are three items that could start
this afternoon.
