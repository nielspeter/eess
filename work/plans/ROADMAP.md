...
| [0096 — dogfood missing crossvalidate bindings](./0096-dogfood-missing-crossvalidate-bindings.md) | High     | Ready | mount `md↔gherkin` + `md↔mermaid` in `check:crossval` on the repo's own artifacts (plant a real `scenario-binding.feature` citation; validate embedded `mermaid` fences) — the project dogfoods 5 of 7 bindings, not 3; `md↔mermaid-er` parked (no erDiagram)         | — buildable now                                                                        |
