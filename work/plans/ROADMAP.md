...
| [0165 — integrate the copied ts-archunit engine](./0165-integrate-the-copied-ts-archunit-engine.md) | High     | Draft | All 3 phases done — engine failures 0; kernel re-split (30 modules, `check:family` non-vacuous); `report`/`--fix` and the full pre-copy export surface restored, `check:nonvacuity` 8→0. Open: 3 ADR decisions, and 204 arch + 11 baseline findings no phase owns | — buildable now, off baseline `9489684`                                                                                                                           |
