...
| [0165 — integrate the copied ts-archunit engine](./0165-integrate-the-copied-ts-archunit-engine.md) | High     | Draft | Phases 1–2 done — engine failures 0; kernel re-split (30 modules, 89/127 files, `check:family` non-vacuous, `.select()` back). Phase 3 (`report`, `--fix`, `havePathMatching`) open, plus an ADR for the 27 still-duplicated modules      | — buildable now, off baseline `9489684`                                                                                                                           |
