...
| [0142 — bind accepted proposals to plans](./0142-bind-proposals-to-plans.md)                       | Medium   | Draft | make `**Ruling:` a literal, parseable field (skill + `PROPOSALS.md` + five real files); gate proposal→plan linkage in `check:corpus` via a declared back-reference — closes what [bug 0141](../bugs/0141-no-check-binds-accepted-proposals-to-plans.md) diagnosed, after its own six-persona review found the naive Fix unbuildable | — buildable now                                                                        |
