...
| [0088 — fold ts-archunit 0.59 into eess](./0088-fold-ts-archunit-into-eess.md)               | High     | Ready | fold its 0.59 engine back into kernel + eess-ts; port ts-archunit ADR-008/009 as eess ADR-009/010 (the fail-closed doctrine); author the breaking release as merged changesets; correct the stale "eess is ahead" record                  | — buildable now                                                                        |
...
**0088 is now `Ready`** (frozen 2026-08-14) — the board's next move is
`/plan-build` on it, or freezing 0089 alongside it.
