...
| [0101 — sibling gates go fail-closed](./0101-sibling-gates-go-fail-closed.md)                    | High     | Ready | opt the four sibling dogfood gates into the folded honest-gate seam — classify every silent-empty selection (`.expectEmpty()`, fix, or in-scope-with-reason), zero baselines; resolve bug 0131 (`check:ledger` can't inherit the fold); file what the honesty surfaces as its own bugs | — buildable now, in progress                                                           |
...
verified, ADR-010's Enforcement table corrected to match. PR #67 merged to
`main` the same day, so 0101's blocker is genuinely satisfied — frozen to
`Ready` and in progress. 0100's "three code plans merged" is not yet met
(0089/0101 still unmerged). Four Important-tier findings 0088's own review
disclosed and never gave a home are now
