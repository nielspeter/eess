...
| **C** — a rule banning direct emitter calls outside rule/test files               | **Accepted, reshaped** | [plan 0237](../plans/completed/0237-eess-runtime-use-only-in-rule-files.md) (Draft, 2026-09-03) carries both narrowings this row required: drop `dispatchRule` from the banned pattern (it is the sanctioned preset-authoring call, used correctly at `packages/md/src/rules/adr.ts:131-141`), and widen the exemption past `*.rules.ts`/`*.test.ts` to cover gate scripts, a shape this repo ships five of. Ships `@nielspeter/eess-ts` minor; the changelog must name the rule id and the `overrides` opt-out                                                                                                                                                           |
