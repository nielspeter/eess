...
| [007 — ts: expose a rule's declared requirements and exclusions](./007-ts-expose-terminalbuilder-declared-state.md)      | Medium   | 🔵 Reviewed | Rewrite needed     | inbound · consuming project                                   | none yet. Two accessors on `TerminalBuilder`/`TsconfigBuilder`; the consumer reads the private fields today and its own comments name "switch to it and delete this" as the exit condition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| [008 — md: a citation form for a bare, non-file identifier](./promoted/008-md-adr-citation-form-for-bare-identifiers.md) | Low      | 🔵 Reviewed | Docs-only          | inbound · consuming project                                   | ✅ **Promoted** → [bug 0236](../bugs/fixed/0236-the-compose-path-for-a-custom-citation-form-is-undocumented.md), which declares `**Implements:** proposal 008` and owns the recipe this ruling called for: `rows()` into `correspondence({ direction: 'both' })` against a caller-held set, in `docs/markdown.md`, with a pointer from the preset's docstring. Promoted 2026-09-03, the day of the ruling. Two findings the review surfaced outrank the ask and are not its owners: [0111](../bugs/0111-md-adr-citations-resolve-by-prefix.md) (the resolver matches by prefix) and [0234](../bugs/0234-adr-citations-resolve-has-no-nonvacuity-fixture.md) (no non-vacuity fixture)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| [009 — core: a verdict cannot be assembled by hand](./009-core-a-verdict-cannot-be-assembled-by-hand.md)                 | High     | 🔵 Reviewed | Split and sequence | inbound · consuming project                                   | Ask A → [plan 0235](../plans/completed/0235-the-emitter-takes-a-receipt.md), Ask C → [plan 0237](../plans/completed/0237-eess-runtime-use-only-in-rule-files.md), both Draft; D still `Held` for a docs owner. Engages [ADR-008](../../adr/008-caller-owns-reporting.md) (keeps `finishPreset`/`reportViolations` on the root), [ADR-010](../../adr/010-a-pass-is-constructed-from-evidence.md) (the guarantee it extends to the emitter seam) and [ADR-011](../../adr/011-the-kernels-public-api-is-explicit.md) (whose first cut moved the two seams to `/internal` and whose review sent them back — not re-asked). Four asks (A–D); `Split and sequence` is the expected ruling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [010 — ts: performance at scale](./010-ts-performance-at-scale.md)                                                       | Medium   | 🔵 Reviewed | Split and sequence | inbound · consuming project                                   | Reviewed 2026-09-06 (`Rewrite needed`); rewritten the same day by the coordinator session against the review; reviewed again the same day by four lenses (`Split and sequence`, every code ask `Held`, conditional on in-place corrections); **corrected the same day, third pass**: A's pre-selection restated as the set-valued dual of `isDeadGlobTree` with per-root matching and fallback-on-empty, C restated around a target-side bound-names field on `ModuleEdge` with its three deltas declared, B1 on both `Predicate` and `Condition` with the registry cap engaged, F `Rejected` with nothing claimed filed, the priority re-rating and the review's own miscounts recorded, the first draft appended as Appendix A, the profile script corrected (the checker build was hiding inside the edge walk) and given an external-project mode. One adopter measurement is now on record, unnamed. Surfaced [bug 0265](../bugs/fixed/0265-a-barrel-reports-a-re-export-at-another-files-line.md); the docs slice is owned by [bug 0266](../bugs/fixed/0266-changed-is-documented-as-scoping-the-run.md) (`Implements: proposal 010`). Stays `Draft`; cannot be `Promoted` while E is held. Awaiting a third review |
| [011 — core: name the evidence gate](./011-core-name-the-evidence-gate.md)                                               | Medium   | 🔴 Draft    | —                  | self-found · architect review of PR #118                      | `withEvidenceGate` (`packages/core/src/report.ts:100`) is private, so every caller that wants a gated receipt without emission reaches it as `finishPreset(x, { report: 'return' })` — measured at 3 CLI doors, `checkAll`, `presets/shared.ts` and both exits of three dogfood scripts. No false green; ADR-008 is satisfied at every site. The complaint is that ADR-014's load-bearing seam is reachable only as a side effect of another function's options object. Supersedes the `verdictOf` primitive the same lens proposed and then withdrew: `verdictOf` is the merge shape, and the merge cannot attribute — which is what PR #118 removed from the CLI. Folds in the `collectViolations` residual PR #118 states rather than closes                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| [012 — a family config declares the artifact graph](./012-a-family-config-declares-the-artifact-graph.md)                | High     | 🔵 Reviewed | Rewrite needed     | self-found · the CLI question, followed to its prior question | Five dialects, one kernel, and no way to say what a project's eess configuration IS. Measured here: 19 `check:*` gates, of which 4 invoke a dialect CLI; 2 of 6 packages ship a bin; the consuming project carries 13 of its own. Each driver re-implements reporting — 25 print/exit lines in `check-corpus.mjs` alone — and the one well-tested CLI was found printing nothing under CI's default format. The deeper ask: a config naming rule files cannot express that a Gherkin feature is implemented, or that an artifact is bound to NOTHING — the green-because-undeclared class. The machinery exists (`correspondence()`, 7 crossvalidate edges, `mergeCollectResults`, and a `scenario↔test (every scenario is proven by a test)` check that already refuses an orphan); the gap is a declaration. Prior question is Ask A and it is the author's to answer                                                                                                                                                                                                                                                                                                                                                   |
...
- **~~A `Docs-only` ruling creates no owner.~~** Closed 2026-08-23 by
  ...
  read as settled.
- **`Split and sequence` still creates no obligation**, by the same argument — it names a
  remedy ("split before planning") and no owner. So does a remainder "deferred behind plan
  NNNN" in prose, which is what 002 had until 0090 declared it. Both are prose, and a rule
...
| rule                                     | fires when                                                                            |
| ---------------------------------------- | ------------------------------------------------------------------------------------- |
| `proposal-states-no-acceptance-criteria` | a live proposal has no level-2 `## Acceptance criteria` heading                       |
...
| `proposal-board-*` (five)                | the board and the files disagree                                                      |
