...
| [009 — core: a verdict cannot be assembled by hand](./009-core-a-verdict-cannot-be-assembled-by-hand.md)                 | High     | 🔵 Reviewed | Split and sequence | inbound · consuming project | none yet. Engages [ADR-008](../../adr/008-caller-owns-reporting.md) (keeps `finishPreset`/`reportViolations` on the root), [ADR-010](../../adr/010-a-pass-is-constructed-from-evidence.md) (the guarantee it extends to the emitter seam) and [ADR-011](../../adr/011-the-kernels-public-api-is-explicit.md) (whose first cut moved the two seams to `/internal` and whose review sent them back — not re-asked). Four asks (A–D); `Split and sequence` is the expected ruling                                                                                                                                                                                                                                                                                                            |
