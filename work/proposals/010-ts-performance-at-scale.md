...
### Dispatch — 2026-09-06, after the third pass

Recorded here because dispositions are a dispatch record, not a verdict, and the author decides them (the coordinator advised; the author said go).

- **Docs slice → [bug 0266](../bugs/fixed/0266-changed-is-documented-as-scoping-the-run.md), filed and fixed in one PR** (the 0236 shape): the `--changed` sentence now says it filters the report, `docs/cli.md` carries a "Large projects" note naming the tsconfig lever and its cost, and ADR-002's enforcement table carries a row for its three promised mitigations, status `deprecated` — measured and not kept.
- **A → `Rejected`, not built.** Open Question 4 answered by the author: 0 files pruned on this repository, ≤2.2% anywhere, negative for a mixed suite, six memo walks against one. The ADR-002 row is the record of the decision, so the ask does not return as "the mitigation ADR-002 promised".
- **C, B, E** stay `Held` as the second review left them; C awaits a review of its corrected mechanism before a plan is cut, B awaits an ADR-writing plan, E its unhold condition.
- The proposal stays `Draft` in place and cannot be `Promoted` while E is held.
