...
| [014](./adr/014-the-emitter-refuses-a-verdict-without-evidence.md) | Evidence at Emission | ADR-010's evidence requirement extends from the terminal to the emitters and to every builder's `violations()`: the receipt is an `ArchViolation[]` carrying `examined` and its declarations, one kernel constructor and one fail-closed merge, and an evidence-free verdict is a configuration finding that throws on every door that hands nothing back. A required field, not a registry. (Accepted 2026-09-03; amended the same day, and again 2026-09-07) |
| [015](./adr/015-the-kernel-extends-resolution-never-execution.md)  | Loading a Rule File  | Module resolution may be extended; module execution may not be delegated to a second registry. The kernel may teach the host resolver TypeScript's `.js` → `.ts` substitution; the file is then imported by Node, into the CLI's own registry. (Accepted 2026-09-12)                                                                                                                                                                                           |
...
examined anything. For that, run the suite, `check:nonvacuity` (the gate prints its own fixture count on every run — this line used to pin the integer, and it was stale again within a fortnight, which is the same lesson the table above records about itself), or
`check:vacuity`.
