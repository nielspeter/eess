...
| Every hand-assembled check in this repo supplies evidence                      | 2    | Not a rule over `scripts/**` — those files sit in no TypeScript project, so such a rule would select nothing, and over `packages/*/src` the compiler already enforces the field. One break-the-loop fixture per hand-assembled check, run on the default path: `check:nonvacuity`'s `emitter/one-dead-check` (`check:corpus`), `emitter/ledger-dead-check` (`check:ledger`) and `emitter/release-dead-check` (`check:release`), each planting the corruption in the production script and asserting `emitter/pass-without-evidence` by id while the other checks still examined. **Plan 0263 Phase 1 had to build the mechanism before the fixture: measured, only `scripts/check-corpus.mjs` built a receipt, and the other two hand-printed on the default path, so a dead check there exited 0 under a green tick.** Two members of `check:release`'s receipt declare from the count they report and are named in that file as not sabotage-detectable                    | `gated`   |
