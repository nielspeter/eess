...
| A rule file exporting an evidence-free builder reds `check`, `baseline` and `checkAll` | 2    | The gate runs **per builder**, where the receipt and the rule file are both in hand: `packages/ts/src/cli/commands/check.ts`, `packages/ts/src/cli/commands/baseline.ts` (which refuses to accept such a finding into a baseline and exits 1), and `packages/ts/src/core/check-all.ts`. `check:nonvacuity`'s `emitter/bare-builder-reds-the-cli` drives the real binary over a probe rule file, asserting `emitter/no-receipt` **by id and by the file it names**, and asserting an honest builder stays green. `packages/ts/tests/core/check-all.test.ts` · `it('throws on a builder loaded from a rule file that hands back a bare array')` covers the other door through the same loader production uses. **Scope, stated because an earlier version of this row said "the CLI" and three of its commands did not red:** `check --fix` returns before any verdict and `doctor` is a diagnostic that reports rules which cannot enforce, so neither is in this clause.                                                                                                                                                                                                                                                                                            | `gated`   |
