...
**Which shape a door uses is decided by attribution, not by taste: a door that
can name the member gates each member and never bundles; a door that cannot,
merges.** `checkAll` receives an anonymous array and merges. The `eess-ts` CLI
