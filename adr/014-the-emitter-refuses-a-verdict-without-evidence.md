...
[ADR-008](./008-caller-owns-reporting.md), which carries the amendment. When this was accepted, every
Enforcement row below was `pending` and owned by
[plan 0235](../work/plans/completed/0235-the-emitter-takes-a-receipt.md), on the
principle that decided is not enforced and this document claims nothing the build
has not yet made true. That principle stands; the tally has moved. As of
2026-09-07 the table is mostly `gated`, and the rows still `pending` are owned by
[plan 0263](../work/plans/0263-adr-014s-residual-enforcement-rows.md), not 0235.
The row's own `Status` column is the authority — this paragraph is history, and a
count here would go stale the way the sentence it replaces did.
...
a merge over zero members is zero examined, undeclared.

**Amended 2026-09-07 (plan 0263 Phase 2). Which shape a door uses is decided by
attribution, not by taste: a door that can name the member gates each member and
never bundles; a door that cannot, merges.** `checkAll` receives an anonymous array and merges. The `eess-ts` CLI
commands still hold the rule file each builder came from, so they gate per
builder and the finding names that file — bundling first would destroy the only
identity the reader can act on, which is bug 0026's seam and was measured on
this ADR's own first implementation. Both shapes make a rule file exporting
`{ violations: () => [] }` red rather than green: it arrives at the merge as a
bare member, or at the gate as a bare receipt attributed to its own file.

The rule has a consequence at the reporting layer worth stating, because it was
also measured wrong: a per-builder door produces one finding **per** dead
builder, so the collapse in `dedupeConfigFindings` must not merge them. That
collapse exists for a preset fanning one authored option out into many rules,
and an emitter finding is not that — it is about a verdict, and each one stands
for a different builder. It cannot be told apart by shape either: an emitter
finding carries `element: ruleId` and `file: ''`, so its key degenerates and
every occurrence in a run merges. Three hand-rolled builders in one rule file
collapsed into one finding whose note said they were "one edit". They are three
edits, and `keyFor` now refuses a key for the four emitter ids. An adopter who sums by hand instead of calling the merge has rebuilt the
...
- **Every builder's `violations()` changes type, and the flagship's own verdict
  paths carry evidence.** `checkAll` aggregates through the merge and the CLI
  commands gate per builder, so a hand-rolled builder in a rule file — the one
  place adopters put builders, loaded without a type-check — cannot pass any of
  them green.
...
| Zero examined at the emitter is a configuration finding, unsuppressable                                                                                         | 2    | Five probes in `scripts/vacuity-matrix.mjs` (`EMITTER_PROBES`) hand the emitters a bare array and a zero-examined receipt; `npm run check:vacuity`. `packages/core/tests/emitter-refuses-without-evidence.test.ts` ("zero examined, zero violations, no declaration is the vacuous-pass finding")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `gated`   |
...
| A rule file exporting an evidence-free builder reds the `eess-ts` CLI's `check`, `check --fix` and `baseline`, `checkAll`, and the `eess-mermaid` CLI's `check` | 2    | Two shapes, both fail-closed. The three CLI doors consult the gate **per builder**, where the receipt and the rule file are both in hand, so the finding names the file it came from: `packages/ts/src/cli/commands/check.ts` (`check` and `--fix`, which loads per file for exactly this reason) and `packages/ts/src/cli/commands/baseline.ts`, which refuses such a finding into a baseline and exits 1. `--fix` additionally refuses **before** `applyFixes`: a fix is an edit derived from a verdict, so a refused verdict refuses every edit computed from it — measured, an earlier cut rewrote a source file under `--apply` while the same run printed `emitter/no-receipt`. `packages/ts/src/core/check-all.ts` has no rule file to name, so it merges its builders' receipts through `mergeCollectResults` and gates the merged receipt, per §7. `check:nonvacuity`'s `emitter/bare-builder-reds-the-cli` drives the real binary over a probe rule file across all three CLI doors in both directions — asserting `emitter/no-receipt` **by id and by the file it names at each of the three**, that `--apply` leaves its target byte-identical, and that an honest builder stays green — and deleting any one of the three gates, the `--fix` attribution, or the refusal-before-write reds it, measured one at a time. (An earlier cut asserted two of the three doors by exit code alone, which the fixture's own rule forbids: a rule file can exit 1 for a dozen reasons.) `checkAll`'s door is the fixture's blind spot and is covered by the suite instead: `packages/ts/tests/core/check-all.test.ts` · `it('throws on a builder loaded from a rule file that hands back a bare array')`, through the same loader production uses. **Scope, stated because an earlier version of this row excused two commands on rationales that were never measured:** `check --fix` was said to return before any verdict; it does not — it calls `violations()` on every builder, and it is in the clause and driven by the fixture. `doctor` is genuinely outside it, because a diagnostic hands back no verdict — but not for the reason given: measured, it prints `No rules that cannot enforce anything.` and exits 0 on this exact input, which is [bug 0268](../work/bugs/0268-doctor-gives-a-clean-bill-to-a-builder-that-enforces-nothing.md), not a justification. | `gated`   |
| A per-builder door names every dead builder, and the collapse does not merge them                                                                               | 2    | `dedupeConfigFindings` keys on `(rule file, rule id, offending glob)`; `keyFor` returns no key for the four ids in `packages/core/src/emitter-findings.ts`, which carry `element: ruleId` and `file: ''` and so would otherwise all share one key. Keyed on the id set rather than on that shape, deliberately: a first cut refused a key whenever the element repeated the identity, and `packages/ts/tests/core/the-floor.test.ts`'s "CONTROL: genuinely identical findings still collapse" reds on it — a real rule with a real narrowing and no glob to name looks identical and genuinely is one edit. `packages/ts/tests/cli/dead-builders-are-named-individually.test.ts` · `it('names every dead builder, rather than collapsing them into one edit')` pins it at the door the clause is about. The kernel keeps its own unit tests for `keyFor` (`packages/core/tests/dedupe-config-findings.test.ts`, "never collapses an emitter finding" and its narrowness control "CONTROL: a non-emitter finding whose element repeats its id still collapses") — cited by path and title rather than in the binding form, because the resolver loads only `packages/ts`'s tsconfig and cannot see a kernel test ([bug 0262](../work/bugs/0262-an-adr-cannot-cite-a-kernel-test.md)). That fixture was corrected in the same change: it had been keying on the rule id, so every collapse assertion in the file was passing over a shape that must never collapse. **Measured before the guard:** three hand-rolled builders in one rule file reported as ONE finding whose note said they were "one edit"; after, three findings and no note. **The other published binary, added by [bug 0269](../work/bugs/fixed/0269-eess-mermaids-check-door-greenlights-a-builder-that-enforces-nothing.md):** `eess-mermaid check` counted the throws from `builder.check()` and never read what a rule examined, so a hand-rolled no-op earned a green tick and a denominator. Its loader now keys on the receipt rather than on `check`, it gates per rule file, and a file contributing no rules reds. `check:nonvacuity`'s `emitter/mermaid-bare-builder-reds-the-cli` asserts all four directions by id and by file. The clause is now true of every dialect that publishes a binary; `eess-md`, `eess-gherkin` and `eess-crossvalidate` publish none.                                   | `gated`   |
...
| Every hand-assembled check that reaches a kernel emitter supplies evidence                                                                                      | 2    | Three scripts hand-assemble findings and hand them to an emitter: `scripts/check-corpus.mjs`, `scripts/check-ledger.mjs`, `scripts/check-release.mjs`. Each builds one receipt from its own checks' receipts, consulted on BOTH exits, and each has a break-the-loop fixture on the DEFAULT path — `check:nonvacuity`'s `emitter/one-dead-check`, `emitter/ledger-dead-check` and `emitter/release-dead-check` — planting the corruption in the production script and asserting `emitter/pass-without-evidence` by id on both exits while the other checks still examined a non-zero count. **Scope, stated because the clause once said "every": five more `check:*` scripts hand-print with no emitter at all** (`check-workspace-integrity`, `check-surface`, `check-docs-code`, `check-review-harness`, `check-nonvacuity`) and are outside this clause. **Ceiling: the receipts guard the RULES, not the inputs** — a shell that mis-parses its own input moves the population and the declaration together, which belongs to that parser's fixtures. **Gated on the second attempt** (plan 0263 Phase 1): the first built the receipts in the shell from borrowed denominators, and five reviews measured that 1 of 9 checks reddened when its own check died                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `gated`   |
...
| A wrong `examined` is not detected                                                                                                                              | 5    | Stated ceiling, not a clause — nor an adopter who sums by hand — nor eess's own two exported `collectViolations` helpers (`packages/core/src/baseline-generator.ts`, which takes throwing `check()` builders, and `packages/ts/src/helpers/baseline-generator.ts`, which takes `violations()` ones): same name, different contract, both ungated, both now carrying the caveat in their docstrings and in `docs/api-reference.md`. nor one who never calls an emitter. Closing the two helpers is proposal 011 ask C. Review-enforced residue: Tier 5 because only a human reading the loop can judge the number; named so it is not mistaken for coverage                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `n/a`     |
