...
| A rule file exporting an evidence-free builder reds the CLI and `checkAll`     | 2    | Both doors merge their builders' receipts with `mergeCollectResults` and consult the gate under `report: 'return'` — `packages/ts/src/cli/commands/check.ts` and `packages/ts/src/core/check-all.ts`. `check:nonvacuity`'s `emitter/bare-builder-reds-the-cli` drives the real CLI over a probe rule file exporting `{ violations: () => [] }` and asserts `emitter/no-receipt` by id; `packages/ts/tests/core/check-all.test.ts` covers the other door with four cases including a declared-empty control. **Both doors were open until plan 0263 Phase 2**: measured, that probe file ran to `"total": 0, "examined": null`, exit 0, and `checkAll` over the same builder returned silently — the phase named only `checkAll`, and the CLI half was a premise nobody had checked                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `gated`   |
