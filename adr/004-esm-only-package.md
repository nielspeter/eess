...
| ------------------------------------------------ | ---- | ------------------------------------------------------------------------------------- | ------ |
| Every package ships `"type": "module"`, ESM only | 1    | `packages/ts/package.json` (and siblings); CI `npm run build` under Node16 resolution | gated  |
| No dual CJS/ESM artifacts                        | 1    | build emits ESM only (`packages/ts/tsconfig.build.json`); pack smoke test             | gated  |
