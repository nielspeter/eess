...
Proposed (2026-08-23). **Prospective** — `@nielspeter/eess/internal` does not
exist today; `packages/core/package.json` ships a single `"."` entry. The
decision lands here ahead of the work, per the house split: an ADR decides, a
plan implements. The classification of every kernel symbol into root or
`/internal` is that plan's, not this ADR's.
...
| ------------------------------------------------------------------------------------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| Every symbol exported from the kernel root appears in `docs/`, a package README, or an ADR | 1    | `check:docs-code`'s public-surface gate — `undocumentedExports` (`scripts/lib/public-surface.mjs:98`), wired into `npm run validate`. The gate is built and runs today, but **not as this clause needs it**: it flattens all six packages — the `PACKAGES` list, flat-mapped at `scripts/lib/public-surface.mjs:96`, so a kernel symbol is counted once per dialect that re-exports it. What is owed is the denominator, not the gate — scoping it to the kernel root, which only means something once the classification exists                                                                                                                                                                                                                                                                                    | `pending` |
| No dialect barrel re-exports a symbol obtained from `@nielspeter/eess/internal`            | 1    | a new rule beside `family/re-export-complete` in `family.rules.ts`, over each dialect's barrel (`packages/ts/src/index.ts` and its siblings). It must resolve reachability with `getExportedDeclarations()`, **not** a scan of the barrel's own `export … from` declarations: a dialect can import from `/internal` in a non-barrel file and re-export it locally or through `export * from './local.js'`, which a same-file scan cannot see. This is recorded in the file the rule would live beside — `reachableExportNames` (`scripts/lib/family-re-exports.mjs:111`) uses the transitive resolver because the direct-only version produced 16 false positives against this repo. A negative assertion built on the narrower scan would reintroduce that gap silently, in the direction that reads as compliance | `pending` |
| Re-export completeness is computed from root imports only                                  | 1    | **No code change is required here, and the row says so rather than implying work.** `kernelImportsOf`'s two specifier tests are already strict equality against `'@nielspeter/eess'` (`scripts/lib/family-re-exports.mjs:83`), not a prefix match, so `@nielspeter/eess/internal` imports are invisible to re-export completeness the moment the subpath exists. `pending` because the subpath does not exist yet — not because this file must change                                                                                                                                                                                                                                                                                                                                                               | `pending` |
...
| Consumers do not import `@nielspeter/eess/internal`                                        | 4    | **None possible.** A subpath export is published and resolvable by any consumer; nothing in npm, TypeScript or this repo can prevent it. The name and its documentation are the whole mechanism, and review is the only backstop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `manual`  |
