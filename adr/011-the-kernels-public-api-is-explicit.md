...
`/internal`. Then read the result and change what it got wrong, which was four
calls: `correspondence` and `CorrespondenceBuilder` (documented on six pages and
the public surface of two dialects), and `reportViolations` and `finishPreset`
(named seams in [ADR-008](./008-caller-owns-reporting.md)). The `ArchJson*` types
