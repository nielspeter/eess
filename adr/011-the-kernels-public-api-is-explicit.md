...
| A dialect publishes the receipt seam an adopter needs                                      | 1    | **The demand half of standalone sufficiency, added 2026-09-10 (bug 0276).** The clause above is the SUPPLY half: a dialect re-exports what its own source imports. That is import-driven by design, so a dialect whose source never touches a symbol owes nothing — correct, and the reason `eess-gherkin` shipped neither receipt constructor while `check:family` stayed green. Nothing asked whether a dialect publishes what an ADR requires an ADOPTER to be able to do. It does now: `scripts/lib/receipt-constructors.test.mjs`, run by `check:family`, asserts every barrel dialect publishes `collectResult`, `mergeCollectResults`, `hasEvidence` (ADR-014's receipt), `reportViolations` (ADR-008's one emitter), `finishPreset` (ADR-014 §the seam for the packages that publish no binary) and `isArchConfigError` (so a caller can recognise what that seam throws). **The set is derived from those clauses, so amending them means amending it** — the first cut derived it from a release's changeset instead and shipped gherkin a receipt it could not hand anywhere, which an architecture review measured. Two cases guard the guard: one asserting every required name is a real kernel root export, one asserting neither list is empty, because both are `filter`ed and an emptied list passes vacuously. **Scope limit, stated:** `eess-crossvalidate` ships flat entry files rather than a barrel, so what its adopter can reach is a per-subpath question — excluded, and owned by [bug 0277](../work/bugs/0277-crossvalidates-flat-entries-have-no-standalone-contract.md) | `gated`  |
