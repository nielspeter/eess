...
| ---------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Confinement: only the engine module imports ts-morph       | 1    | eess-ts import ban `adr007/ts-morph-confined-to-engine` in `adr007.rules.ts` (modules outside `packages/ts/src/core/engine/` must `notImportFrom` ts-morph) — standalone, un-gated because the code does not satisfy it yet (ts-morph imported across ~55 eess-ts files); the honest declared gap this ADR exists to close. Ratchets into `arch.rules.ts` / `gated` when green. | pending |
| Batch-first boundary: coarse-grained, engine-neutral calls | 4    | design review against this ADR's Rule 2                                                                                                                                                                                                                                                                                                                                         | manual  |
