...
Developers who've used Java ArchUnit can transfer their mental model directly.
...
| ------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------- | ------ |
| Rules read as fluent chains: `entry().that().should().check()`      | 2    | vitest · `packages/ts/tests/integration/class-entry-point.test.ts` · it('fluent chain reads naturally') | gated  |
| `.check()` throws `ArchRuleError` carrying the violations           | 2    | vitest · `packages/ts/tests/core/rule-builder.test.ts` · it('includes violation details in the error')  | gated  |
| Wide method surfaces on builder classes are the design, not a smell | 5    | governance — this ADR; cited by the metrics exclusion in `work/dogfood-coverage.md`                     | manual |
