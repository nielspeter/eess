...
**This was an error made then, not ground that moved.** An earlier draft of this
paragraph said plan 0165 had measured a real split on an older `jiti`. That is
unmeasured and false: `package-lock.json` has pinned `jiti` 2.7.0 since the
initial commit, plan 0165 landed under the same version, and the declared range
has never changed. Worse, the corpus already knew —
[bug 0199](../work/bugs/fixed/0199-a-bare-preset-call-throws-before-baseline-filtering.md)
records under "this record got it wrong TWICE" that the same separate-registry
claim is false, disproved three ways including by a fixture that ran "with one
shared registry and no jiti at all". A method review caught the exculpation, and
the distinction matters for the next reader: "ground moved" says plan 0165 is
still trustworthy and a `jiti` upgrade is the risk; "an error made then" says to
re-check its other measurements. The evidence supports the second.
...
| A rule file's accepted language does not vary with the consumer's `type` field  | 2    | **Not gated, and false today.** Break class: _a rule file whose SIBLING holds non-erasable syntax loads under `"type": "commonjs"` and is refused under `"type": "module"`_ — measured both ways on the `engines` floor. Under `commonjs` the entry's ESM syntax trips `isModuleFormatRefusal` before Node ever parses the sibling, so `jiti` transpiles the graph. `packages/ts/tests/cli/esm-sibling-import.test.ts` is `"type": "module"`-only by construction, so nothing reds. Closing it means restricting the fallback transform to erasable syntax; the record is [bug 0281](../work/bugs/0281-mermaid-rule-files-accept-syntax-eess-ts-refuses.md)                                                                                   | `pending` |
