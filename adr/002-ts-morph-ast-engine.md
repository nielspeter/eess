...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| All AST/type work via ts-morph, never the raw `typescript` compiler API                                                                                                | 1    | eess-ts rule `eess/adr002-no-raw-typescript` in `arch.rules.ts`, gated in CI via check:arch                                                                                                                                                                                                                                                                                                                                                                  | gated      |
| TypeScript pinned to ts-morph-compatible range (~5.9)                                                                                                                  | 1    | `package.json` dev pin; CI `npm run typecheck` breaks on drift                                                                                                                                                                                                                                                                                                                                                                                               | gated      |
| Performance mitigations for 5000+ files promised under Negative consequences: lazy source file parsing, predicate memoization, file-set narrowing via `resideInFolder` | 3    | Measured 2026-09-06 by `scripts/profile-ts-check.mjs` (proposal 010): parsing is eager at `packages/ts/src/core/project.ts` and the checker build is a second cost of the same size; per-process memoization ships (`packages/ts/src/core/element-cache.ts`, plan 0075); file-set narrowing was measured at ≤2% of a cold run and 0 files on this repository and **declined** (proposal 010, Ask A). The promise is not kept as written and this row says so | deprecated |
