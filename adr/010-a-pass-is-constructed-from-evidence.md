...
(see Alternatives). No suppression mechanism beyond what Phase 1
classified as kernel-bound (the unforgeable `WeakSet`-backed registries) —
each guards a distinct, named audience, and nothing may add another.

**Amended 2026-09-09 (plan 0263 Phase 4): the arithmetic was wrong.** This read
"no third … nothing may add a fourth", which counts a set of three. Measured,
there are **two** homes: `CARDINALITY_ASSERTERS` in
`packages/core/src/cardinality.ts` and `OWNERS` in
`packages/core/src/owns-empty-discovery.ts`. The gate that makes this clause
enforceable therefore caps at two — and while the sentence said "a fourth", an
author reddened on their third would have read the binding ADR as permitting it
and concluded the gate was wrong. That is ADR-009 rule 1 with this document as
the accomplice, and the ordinal is removed rather than corrected to keep it from
going stale the day a third home is legitimately sanctioned.
...
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Terminal seam requires `{ violations, examined }` evidence to construct a pass        | 1                                                                        | compiler-checked return type: `CollectResult` (`packages/core/src/terminal-builder.ts:29`) is the abstract `collectViolations()`'s return type (`packages/core/src/terminal-builder.ts:344`) — every subclass across every dialect must construct it, or the build does not compile                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `gated` |
...
| The kernel-bound suppression registries are a closed set                              | 1                                                                        | `arch.internal.rules.ts`'s `eess/no-new-kernel-registry`: no module under `packages/core/src/**`, other than the two named homes, constructs a `WeakSet`. Denominator 58 modules examined at authoring, of which 56 are in the enforced set — exclusions filter violations, not the population, so the two homes are inside `examined` and outside what the rule can fail on. The live figures are the rule's to print, not this row's to pin. **Two mechanisms, because the rule alone cannot see two shapes of a new home.** It excludes the homes by path, so a second registry added INSIDE one is invisible, and growing the exclusion list silently exempts a real one — both measured by review. `check:nonvacuity`'s `arch/no-new-kernel-registry` therefore also takes a **census**: exactly two `new WeakSet` under `packages/core/src`, one in each named home, counted independently of the rule's own exclusions — a clause about a population needs a mechanism that counts the population. It asserts by rule id **at error severity** (a `.asSeverity('warn')` downgrade left every other arm green), and holds the `WeakMap` control: `packages/core/src/selection-memo.ts`'s memo caches must stay quiet. | `gated` |
| Zero loaded source files outranks any `.expectEmpty()` declaration                    | 2                                                                        | `TerminalBuilder.evidencedViolations()`'s `sourceEmpty === true` precedence branch (`packages/core/src/terminal-builder.ts:253`), checked _before_ `.expectEmpty()`/`assertsCardinality()`; `packages/ts/tests/core/the-floor.test.ts` · `it('empty project + .expectEmpty() still reports the PROJECT, never the declaration')`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `gated` |
...
| The configuration finding names its cause's remedy, verified to remediate             | 2                                                                        | Both halves now built. Remedy _message_ correctness and cause-specificity: `packages/ts/tests/core/assertion-gate.test.ts` · `it('NO remedy may name a mechanism that does not remediate')`. The "verified to remediate" half — the fixture this row recorded as missing — arrived with plan 0165's engine adoption: `packages/ts/tests/builders/slice-rule-builder.test.ts` · `it('EVERY remedy is true: applying what each message says fixes the rule')` fires each discovery-failure branch, applies the transformation that branch's own message recommends, and asserts discovery recovers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `gated` |
