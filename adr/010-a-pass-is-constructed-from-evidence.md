...
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| Terminal seam requires `{ violations, examined }` evidence to construct a pass        | 1                                                                        | compiler-checked return type on the kernel's terminal seam, built in plan 0088 Phase 4                                                 | `pending` (flips `gated` when Phase 4 lands)  |
| Every published check-constructor fails over a zero-file project (the vacuity matrix) | 2 (behavioral — the harness _executes_ constructors, not a static claim) | vacuity matrix over `package.json` exports → `dist` → namespace-object exports, `KNOWN_FAIL_OPEN` ratchet, built in plan 0088 Phase 4a | `pending` (flips `gated` when Phase 4a lands) |
| Zero loaded source files outranks any `.expectEmpty()` declaration                    | 2                                                                        | instrument-level precedence check ahead of declaration lookup, plan 0088 Phase 4                                                       | `pending` (Phase 4)                           |
| Every surviving eess-ts preset threads an empty-declaration carrier                   | 2                                                                        | preset option review + a committed fixture per preset asserting the carrier reaches every constructed rule, plan 0088 Phase 5          | `pending` (Phase 5)                           |
| The configuration finding names its cause's remedy, verified to remediate             | 2                                                                        | a committed violating fixture per cause (empty project / dead selector / over-filtered), plan 0088 Phase 5                             | `pending` (Phase 5)                           |
