...
| No `as` casts outside documented interop boundaries                         | 1    | eess-ts rule `eess/adr005-no-type-assertions` in `arch.internal.rules.ts`, gated in CI via check:arch; boundaries sanctioned inline with reasons | gated  |
| No non-null assertions (`!`) outside documented boundaries                  | 1    | eess-ts rule `eess/adr005-no-non-null` in `arch.internal.rules.ts`, gated in CI (zero sanctions — all sites real-fixed)                          | gated  |
| eslint-disables only at unavoidable JS interop boundaries, with explanation | 4    | human review; each disable carries a reason comment                                                                                              | manual |
