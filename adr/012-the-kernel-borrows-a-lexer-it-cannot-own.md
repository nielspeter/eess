...
| A reason-free waiver applies, and becomes an unsuppressable finding       | 2    | `packages/ts/tests/core/exclusion-comments-reach-every-condition.test.ts` · `it('an undocumented exclusion fails the build (bug 0039)')` pins it for the ts path; `packages/core/tests/exclusion-reason-and-nesting.test.ts` pins the kernel half that all five dialects now share (by file — see the row above for why a kernel test cannot be cited by title)                                                                                                                                                                                      | `gated` |
