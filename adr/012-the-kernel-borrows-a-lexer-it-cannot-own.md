...
| A markdown file with no injected masker gets the conservative default             | 2    | `packages/core/tests/mask-markdown.test.ts` pins top-level shapes only. No test covers a fence in a list item or blockquote, or HTML, and the property is measured false there ([bug 0291](../work/bugs/0291-the-markdown-masker-honours-a-directive-inside-code.md), [bug 0293](../work/bugs/0293-an-example-inside-an-html-block-is-read-as-prose.md))                                                                                                                                                                                                                                                                                                                                                                                                | `pending` |
| The identity masker leaves the default's refusals unchanged                       | 2    | `packages/core/tests/exclusion-mask-injection.test.ts` — injects the identity function as a masker and asserts a directive inside a string literal is still refused. Verified by sabotage: replacing the composition with the injected masker alone reds exactly that test and no other. **Cited by file, not by `it()` title, and not by choice**: `check:crossval` resolves ADR test citations against `project('packages/ts/tsconfig.json')`, so no clause in any ADR can name a kernel test by title. The gap is the mechanism's, not this row's                                                                                                                                                                                                    | `gated`   |
| An injected masker cannot make the parser accept what the default refuses         | 2    | No mechanism holds this, and it is measured false for code-like and markdown files on `main`'s kernel ([bug 0291](../work/bugs/0291-the-markdown-masker-honours-a-directive-inside-code.md)); see the property 3 correction below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `pending` |
...
**Why.** The default markdown masker decides what is code with a line loop that
cannot see containers. [Bug 0291](../work/bugs/0291-the-markdown-masker-honours-a-directive-inside-code.md) records the shapes by value.
...
- **The property itself** is now a `pending` row, with no mechanism. A dialect's
  masker has to be tested directly; composition cannot bound it.

Recorded here rather than edited away, like the correction above.

The Consequences sentence saying a dialect can inject a bad masker "but not an unsafe one" is false by this measurement.
