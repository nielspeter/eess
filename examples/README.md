...
| [cross-dialect.gherkin-ts.test.ts](./cross-dialect.gherkin-ts.test.ts) | Gherkin↔TS       | [fixtures/gherkin-ts/red/](./fixtures/gherkin-ts/red/) and [fixtures/gherkin-ts/covered/](./fixtures/gherkin-ts/covered/) — a dangling path, an ambiguous suffix, a missing scenario, an uncovered scenario, and a stale exemption, each intentionally broken. Exercises all three README-documented exports: `scenarioTestsResolve`, `scenariosCovered`, `scenarioExemptionsCurrent`. |
